...
class ScoreModuleFunction(BaseModuleFunction):
    ...
    def forward(self, forward_outputs, minibatch, tokenized_contexts, current_minibatch_ids, return_logprobs=False,
                **kwargs):
        if self.llm_config.model_type == "causal":  # hence input should be removed from result
        ...
        if return_logprobs:
            masked_token_probs = tokens_probs.masked_fill(mask, 0.0)
            minibatch_probs = masked_token_probs.sum(-1)
        else:
            masked_token_probs = tokens_probs.masked_fill(mask, 1.0)
            minibatch_probs = masked_token_probs.prod(-1)
